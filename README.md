...
- Twenty-eight settings offer plainly labelled reversible choices. The display language, region, time zone, and date and time format each offer a list of choices rather than a pair. Widgets removal is the one target with no way back. The six tool cards offer **Installed** and **Update installed tool**; neither removes a tool. Each can be left alone by turning off its selection switch.
...
- While a plan runs, the sections can still be clicked through and scrolled, so each card can be watched as it finishes. The action buttons, the restart choice, every card's selection switch, and every card's list of choices are locked, so nothing can be changed part way through a run.
- A setting that cannot finish until you sign out and back in is announced in a dialog you have to dismiss, and the summary line turns red, as well as naming the setting in the summary text.
- The window reports the administrator step as it runs: which setting is being applied, how many are finished, how long the current one has taken, and an unanswered Windows approval prompt named as such. Each card says whether it is finished, running, or still waiting.
- **Stop** ends the administrator step. The elevated worker cannot be killed by the window that started it, so the request is passed to it in a file and taken between settings, never halfway through one. Changes already applied stay applied and the usual results are still shown. Closing the window offers the same stop.
- A display language with no pack on this computer is a Windows Update download of about ten minutes. Dingo says so on the card before you apply, starts the download before the rest of the plan so the quick settings do not queue behind it, and applies that setting last. `Install-Language` reports no percentage, so Dingo watches the DISM and CBS logs, the Windows Update download folder, and the servicing processes instead. If none of them move for five minutes it stops early and says so, rather than waiting out the fifteen-minute limit.
